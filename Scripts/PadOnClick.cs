...
public class PadOnClick : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKey("j") && !hasPressedJ && NPC1.hasSpoken)
        {
            ...
            newJumpPad.AddComponent<Rigidbody>();
    ...
    void OnGUI()
    {
        ...
        {
            if (countPads <= 14)
                GUI.Box(new Rect(20, 160, 200, 30), "Jump Pads Remaining: " + (15 - countPads), thisStyle);
            else if (countPads > 14)
                GUI.Box(new Rect(20, 160, 250, 30), "No more jump pads available.", thisStyle);
        }
    }
}
